...
const MySentInvits = () => {
    ...
    let getMyInvits = React.useCallback(async () => {
    ...
    }, [getAccessTokenSilently, getMyInvits, serverUrl]);


    return (
        <>
            {invitsS.map(invit => <Invitation invit={invit} updateParent={getMyInvits} key={invit.t2.id} />)}
        </>
